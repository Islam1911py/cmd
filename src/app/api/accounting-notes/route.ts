...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectId, unitId, description, amount } = body
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!noteId || !sourceType) {
      return NextResponse.json(
        { error: "Missing required fields: noteId, sourceType" },
        { status: 400 }
      )
    }
    ...
    if (sourceType === "PM_ADVANCE" && pmAdvanceId) {
      ...
      if (!pmAdvance) {
        console.error("❌ PM Advance not found:", pmAdvanceId)
      ...
      console.log("✓ PM Advance validated:", { id: pmAdvanceId, remaining: pmAdvance.remainingAmount })
    } else if (sourceType === "PM_ADVANCE" && !pmAdvanceId) {
    ...
    const expense = await prisma.operationalExpense.create({
      data: {
        ...
        amount: note.amount,
        sourceType: sourceType === "PM_ADVANCE" ? "PM_ADVANCE" : "OFFICE_FUND",  // Use correct enum
        unitId: note.unitId,
    ...
    console.log("✓ Operational expense created:", { id: expense.id, sourceType: sourceType === "PM_ADVANCE" ? "PM_ADVANCE" : "OFFICE_FUND", amount: expense.amount })

    // Deduct from PM Advance if applicable
    if (sourceType === "PM_ADVANCE" && pmAdvanceId) {
      console.log("💰 Deducting from PM Advance:", { pmAdvanceId, amount: note.amount })
