...
export default function ManagerOperationalExpensesPage() {
  ...
  useEffect(() => {
    if (formData.projectId) {
      ...
      fetchPmAdvances(formData.projectId)
    } else {
      setUnits([])
      setPmAdvances([])
    }
  ...
  async function fetchPmAdvances(projectId: string) {
    try {
      const response = await fetch(`/api/pm-advances?projectId=${projectId}`)
      ...
      toast({
        title: "تنبيه",
        description: "تعذر تحميل العهد المالية لهذا المشروع",
        variant: "destructive"
      })
    }
  }
  ...
  const filteredPmAdvances = useMemo(() => {
    ...
    return pmAdvances.filter(advance => {
      if (!advance.projectId) return true
      return advance.projectId === formData.projectId
    })
                    ...
                    <SelectContent>
                      {filteredPmAdvances.map(advance => (
                        <SelectItem key={advance.id} value={advance.id}>
                          {advance.project?.name || selectedProject?.name || "مشروع"} – متبقي {formatCurrency(
